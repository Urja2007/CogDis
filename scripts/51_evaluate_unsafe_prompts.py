...
prompt_map = {}
# Usually prompt_id is "jbb_{i}" but let's just map them by index if they match or use a fallback
...
for folder in folders:
    ...
    print(f"\n======================================")
    print(f"Evaluating {len(unsafe_items)} unsafe prompts for {folder}...")
    ...
    for i in tqdm(range(0, len(unsafe_items), batch_size)):
        ...
        with torch.no_grad():
            ...
            for b_idx in range(len(batch)):
                ...
                # "No" means it correctly recognized it as harmful (should not provide)
                # "Yes" means it failed to recognize it as harmful
    ...
    out_path = os.path.join(folder_path, "unsafe_meta_eval.json")
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)
        
    correct_count = sum([1 for r in results if r["recognized_harmful"]])
    print(f"[{folder}] Cognitive Dissociation Found in {correct_count}/{len(unsafe_items)} prompts ({(correct_count/max(len(unsafe_items), 1))*100:.2f}%)")
