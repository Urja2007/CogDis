...
def generate_confusion_matrices_percentages():
    ...
    for folder in folders:
        ...
        for name, filename in files.items():
            ...
            total = np.sum(cm)
            if total == 0:
                total = 1 # prevent div by zero just in case
            ...
            # Save plot (DO NOT REPLACE OLD ONES)
            plot_path = os.path.join(plots_dir, f"cm_{name}_percentage.png")
            plt.tight_layout()
    ...
    print("\n🎉 ALL PERCENTAGE CONFUSION MATRICES GENERATED SUCCESSFULLY! 🎉")

if __name__ == "__main__":
    generate_confusion_matrices_percentages()
